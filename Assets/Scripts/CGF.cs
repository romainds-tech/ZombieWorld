...
namespace CircularGravityForce
{
    ...
    public class CGF : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("Applies�counter gravity.")]
        private bool counterGravity = false;
        public bool CounterGravity
        {
            get { return counterGravity; }
            set { counterGravity = value; }
        }

        [SerializeField, Tooltip("Counter gravity multiplier.")]
        private float counterGravityMultiplier = 1f;
        public float CounterGravityMultiplier
        {
            get { return counterGravityMultiplier; }
            set { counterGravityMultiplier = value; }
        }

        [SerializeField, Tooltip("Lerps between current objects velocity.")]
        private bool lerpVelocity = false;
        public bool LerpVelocity
        {
            get { return lerpVelocity; }
            set { lerpVelocity = value; }
        }
        ...
        {
            if (rigidbodyCollection == null)
                rigidbodyCollection = new Dictionary<Rigidbody, bool>();
            ...
            if (colliderList != null)
            {
                for (int i = 0; i < colliderList.Length; i++)
                {
                    ...
                    {
                        if (colliderList[i].attachedRigidbody != null)
                        {
                            if (rigidbodyCollection.ContainsKey(colliderList[i].attachedRigidbody))
                            {
                                rigidbodyCollection[colliderList[i].attachedRigidbody] = false;
                            }
                        }
                    }
                }
            ...
            if (raycastHitList != null)
            {
                for (int i = 0; i < raycastHitList.Length; i++)
                {
                    ...
                    {
                        if (raycastHitList[i].collider.attachedRigidbody != null)
                        {
                            if (rigidbodyCollection.ContainsKey(raycastHitList[i].collider.attachedRigidbody))
                            {
                                rigidbodyCollection[raycastHitList[i].collider.attachedRigidbody] = false;
                            }
                        }
                    }
                }
        ...
        private void RigidbodieCollectionUpdate(Rigidbody rigid)
        {
            ...
            if (!rigidbodyCollection.ContainsKey(rigid))
                rigidbodyCollection.Add(rigid, true);
            else
            {
                if (!rigidbodyCollection[rigid])
                    rigidbodyCollection[rigid] = true;
            }
        }
        ...
        private void ApplyForce(Rigidbody rigid, Transform trans, Collider coll)
        {
            ...
            switch (_forceType)
            {
                ...
                case ForceType.Velocity:
                    ApplyCGFVelocity(rigid, transPos, trans);
                    break;
                case ForceType.AngularVelocity:
                    ApplyCGFAngularVelocity(rigid, transPos, trans);
                    break;
            }
        }
        ...
        private void ApplyCGFForceAtPosition(Rigidbody rigid, Vector3 transPos, Transform trans)
        {
            ...
            if (CounterGravity)
            {
                if (rigid.useGravity)
                {
                    rigid.AddForce(-(Physics.gravity * CounterGravityMultiplier), ForceMode.Acceleration);
                }
            }
        }
        ...
        private void ApplyCGFVelocity(Rigidbody rigid, Vector3 transPos, Transform trans)
        {
            ...
            if (LerpVelocity)
                rigid.velocity = Vector3.Lerp(rigid.velocity, force, Time.deltaTime);
            else
        ...
        private void ApplyCGFAngularVelocity(Rigidbody rigid, Vector3 transPos, Transform trans)
        {
            ...
            rigid.maxAngularVelocity = _forceTypeProperties.TorqueMaxAngularVelocity;
            if (LerpVelocity)
                rigid.angularVelocity = Vector3.Lerp(rigid.angularVelocity, torque, Time.deltaTime);
            else
                rigid.angularVelocity = torque;
                ...
                for (int i = 0; i < colliderListCount; i++)
                {
                    if (colliderList[i] != null)
                    {
                        if (!colliderList[i].isTrigger)
                        {
                            ...
                            if (rigid != null && RigidbodieCollectionValidate(rigid))
                            {
                                ...
                                RigidbodieCollectionUpdate(rigid);
                            }
                        }
                    }
                }
